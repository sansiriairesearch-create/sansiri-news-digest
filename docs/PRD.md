...
| **FR-04** | กรองตามช่วงเวลา: **เมื่อวาน 07:00 → วันนี้ 06:00 (Asia/Bangkok)** — คำนวณโดย `daily_meta.py` (`window_start`/`window_end`) และ**ต้องส่งให้ parser ทุกครั้ง** (ถ้าไม่ส่ง = ไม่กรอง = ผิด ดู DECISIONS D-07) | |
